...
import numpy as np
import tensorflow as tf
model = tf.saved_model.load("D:/ObjectDetection_TensorFlow2/my_model_house_1/saved_model")
...
def load_labels_map(label_url):
    ...
    if data:
        return data['labels']
    return []
label_maps = load_labels_map('label_map.pbtxt')
...
img = cv2.imread('D:/ObjectDetection_TensorFlow2/test_house.jpg')
...
output = process_output(results, 0.4, targetSize, label_maps)

img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
boxes = np.array(output['house'])
...
cv2.imwrite('test_house_1.jpg' , img)
key = cv2.waitKey(0)